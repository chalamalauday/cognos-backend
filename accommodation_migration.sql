-- COGNOS 2K26 accommodation upgrade for an existing database
-- Run this once in phpMyAdmin after selecting the cognos_2k26 database.
-- Do not run this file if you are importing the complete database.sql from scratch.

ALTER TABLE `registrations`
  ADD COLUMN `gender` ENUM('Boys', 'Girls') NULL AFTER `college_name`,
  ADD COLUMN `distance_from_college_km` DECIMAL(7,2) NULL AFTER `gender`,
  ADD COLUMN `accommodation_required` TINYINT(1) NOT NULL DEFAULT 0 AFTER `distance_from_college_km`,
  ...
  ADD INDEX `idx_accommodation` (`accommodation_required`, `gender`);
...
-- Existing registrations do not have accommodation answers.
-- Review them in the admin dashboard before making accommodation allocations.
-- After reviewing, these columns can be made mandatory for future records:
-- ALTER TABLE `registrations`
--   MODIFY `gender` ENUM('Boys', 'Girls') NOT NULL,
--   MODIFY `distance_from_college_km` DECIMAL(7,2) NOT NULL;
